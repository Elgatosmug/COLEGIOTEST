...
import datadeportes from '../contexts/deportesdata.json';
...
export function Tabla() {
        ...
        {textArray.map((text, index) => {
          ...
          return (
            <div
              key={index}
              style={{
                marginBottom: index < textArray.length - 1 ? "8px" : "0",
              }}
            >
  ...
  // Función para renderizar celdas con separación de filas opcional
  const renderFlexibleCell = (data, type = 'default') => {
    // Si es un string simple, renderizar directamente
    ...
    // Verificar si es estructura compleja (array de objetos)
    const isComplexStructure = Array.isArray(data) &&
      data.length > 0 &&
      typeof data[0] === 'object';

    if (isComplexStructure) {
      // Renderizar con separación de filas
      if (type === 'cierre') {
        return (
          <div style={{ width: '100%' }}>
            {data.map((item, index) => (
              <div
                ...
                style={{
                  ...
                  minHeight: '50px', // altura uniforme
                  display: 'flex',
    ...
    // Renderizar estructura simple (arrays o strings)
    return (
      <div style={{
        whiteSpace: 'pre-line',
        lineHeight: '1.4',
  ...
  const columns = [
    {
      name: "Deporte",
      cell: (row) => formatTextWithBold(row.deporte, ["NATACIÓN", "TRIATLÓN", "AJEDREZ", "BASQUET", "BASQUETBOL", "MINI", "ATLETISMO", "VOLEIBOL", "PLAYA", "SALA", "FUTSAL", "5", "7", "9", "FUTBOL", "11", "GIMNASIA", "ARTISTICA", "TENIS", "DE", "MESA", "TAE", "KWON", "DO", "KARATE", "JUDO", "KICK", "BOXING"]),
      grow: 1 // Más ancho relativo
    },
    {
